...
public class ReservaService {
    ...
    private void verificarPermissaoProfessor(TipoUsuario tipo) {
    ...
    public Reserva reservar(TipoUsuario tipo, String professor, String laboratorioId,
                            String dataStr, String horaInicioStr, String horaFimStr, String turma) {
        verificarPermissaoProfessor(tipo);
    ...
    public List<Reserva> listarReservasFuturas(String professorNome) {
        LocalDate hoje = LocalDate.now();
        LocalTime agora = LocalTime.now();
        List<Reserva> futuras = new ArrayList<>();

        for (Reserva r : reservas) {
            boolean isFutura = r.getData().isAfter(hoje) ||
                               (r.getData().equals(hoje) && r.getHoraInicio().isAfter(agora));

            if (isFutura && r.getProfessor().equalsIgnoreCase(professorNome.trim())) {
                futuras.add(r);
            }
        }

        // Ordena por data e, em caso de empate, por hora de início
        futuras.sort((r1, r2) -> {
            int dateComp = r1.getData().compareTo(r2.getData());
            if (dateComp != 0) {
                return dateComp;
            }
